// App.tsx
// hs31@codingsessions-MacBook-Air ~ % /Applications/MAMP/Library/bin/mysql80/bin/mysql -u root -p
// Password: root
import "./styles.css";
...
export default function App() {
  ...
  const [sortBy, setSortBy] = useState("lastName"); // lastName, firstName, dateOfDeath, branch const [sortOrder, setSortOrder] = useState("asc"); // asc, desc
  ...
  const addVeteran = async (e) => {
    ...
    try {
      ...
      const newVeteran = {
        latitude: veteranForm.latitude
          ? parseFloat(veteranForm.latitude)
          : null,
        longitude: veteranForm.longitude
          ? parseFloat(veteranForm.longitude)
          : null,
        firstName: veteranForm.firstName.trim(),
        middleName: veteranForm.middleName.trim(),
        lastName: veteranForm.lastName.trim(),
        cemeteryName: veteranForm.cemeteryName.trim(),
  ...
  const handleEditVeteran = (veteran) => {
    if (veteran.userId === user?.uid) {
      ...
      setVeteranForm({
        ...
        cemeteryName: veteran.cemeteryName || "",
  ...
  const handleImportFileChange = async (e) => {
      ...
      reader.onload = (event) => {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          if (!row.firstname || !row.lastname) {
            errors.push(`Row ${i}: Missing First Name or Last Name — skipped`);
            continue;
          }
          ...
          preview.push({
            ...
            branch: row.branch || "",
            rank: row.rank || "",
            conflictWar: row.conflictwar || row.conflict || row.war || "",
            dateOfBirth: row.dateofbirth || row.dob || null,
            dateOfDeath: row.dateofdeath || row.dod || null,
            serviceDateStart: row.servicedatestart || row.servicestart || null,
            serviceDateEnd: row.servicedateend || row.serviceend || null,
            latitude: row.latitude ? parseFloat(row.latitude) : null,
            longitude: row.longitude ? parseFloat(row.longitude) : null,
            cemeteryName: row.cemeteryname || row.cemetery || "",
            additionalInfo: row.additionalinfo || row.notes || "",
          });
        }
                ...
                messages: [
                  {
                    ...
                    content: [
                      ...
                      {
                        type: "text",
                        text: `This is a PDF of a veteran cemetery registry exported from Google Sheets.
    
    The sheet has these columns (in roughly this order):
    Block, Latitude, Longitude, Last Name, First Name, Middle Name/Initial,
    Date of Birth, Date of Death, War/Conflict, Branch, Rank, Notes/Photograph
    
    Extract every veteran row and return ONLY a raw JSON array.
    No explanation. No markdown. No code fences. Start with [ and end with ].
    
    Map columns to these exact JSON keys:
    - firstName
    - middleName
    - lastName
    - branch
    - rank
    - conflictWar
    - dateOfBirth  (YYYY-MM-DD if possible)
    - dateOfDeath  (YYYY-MM-DD if possible)
    - latitude     (number or null)
    - longitude    (number or null)
    - cemeteryName (use block/location info, otherwise "")
    - additionalInfo (Notes column if present, otherwise "")
    - serviceDateStart ("")
    - serviceDateEnd ("")
    
    Rules:
    - Skip rows with no Last Name AND no First Name
    - Skip header rows
    - Use "" for missing strings, null for missing lat/lng
    - Return ONLY the raw JSON array`,
          ...
          if (!response.ok) {
            ...
            // CORS or network error guidance
            if (response.status === 0 || errText.includes("CORS")) {
              setImportErrors([
                "CORS Error: The browser cannot call the Anthropic API directly.",
                "Please use one of these alternatives:",
                "1. Export as CSV from Google Sheets: File → Download → Comma Separated Values (.csv)",
                "2. Set up a backend proxy server to handle the API call",
                "3. Use the CSV template (Download CSV Template button above) to manually format your data",
              ]);
            } else {
              setImportErrors([
                `API Error ${response.status}: ${errText.substring(0, 300)}`,
              ]);
            }
            setIsParsing(false);
            return;
          ...
          if (!rawText) {
            setImportErrors([
              ...
              "Try: File → Download → PDF (.pdf) instead of Print → Save as PDF",
            ]);
            setIsParsing(false);
            return;
          }
          ...
          if (!jsonMatch) {
            setImportErrors([
              "Could not find structured data in the AI response.",
              `Response preview: ${rawText.substring(0, 200)}`,
            ]);
            setIsParsing(false);
            return;
          }
          ...
          try {
            ...
            setImportErrors([
              "Could not parse AI response as JSON. Please try again or use CSV.",
            ]);
            setIsParsing(false);
            return;
          }
          ...
          if (!Array.isArray(parsed) || parsed.length === 0) {
            setImportErrors(["No veteran records were found in the PDF."]);
            setIsParsing(false);
            return;
          }
          ...
          parsed.forEach((v, i) => {
            if (!v.firstName && !v.lastName) {
              errors.push(
                `Record ${i + 1}: Missing both First and Last Name — skipped`
              );
              return;
            }
            preview.push({
              ...
              branch: v.branch || "",
              rank: v.rank || "",
              conflictWar: v.conflictWar || "",
              dateOfBirth: v.dateOfBirth || null,
              dateOfDeath: v.dateOfDeath || null,
              serviceDateStart: v.serviceDateStart || null,
              serviceDateEnd: v.serviceDateEnd || null,
          ...
          // This catch fires on network/CORS failures
          setImportErrors([
            "Cannot reach the AI API from the browser (network/CORS restriction).",
            "Best fix: Export your Google Sheet as CSV instead:",
            "1. In Google Sheets: File → Download → Comma Separated Values (.csv)",
            "2. Upload the .csv file here — it works instantly with no API needed",
            "Or ask your developer to add a backend proxy for PDF support.",
          ]);
        }
        setIsParsing(false);
      };
  ...
  const sendChatMessage = async () => {
    ...
    try {
      const veteransSummary =
        veterans.length > 0
          ? `There are ${
              veterans.length
            } veterans in the registry. Examples: ${veterans
              .slice(0, 3)
              .map(
                (v) =>
                  `${v.firstName} ${v.lastName} (${
                    v.branch || "unknown branch"
                  }, ${v.conflictWar || "unknown conflict"})`
              )
              .join("; ")}.`
          : "The veterans registry is currently empty.";

      const systemPrompt = `You are a helpful AI assistant for the Memorial Garden Registry, 
  a digital archive honoring American military veterans. You help users find information 
  about veterans, answer questions about military history, branches of service, conflicts, 
  and assist with managing veteran records. Be respectful and compassionate.
  Current registry context: ${veteransSummary}`;
      ...
      setChatMessages([
        ...
        {
          role: "assistant",
          content: `⚠️ Error: ${error.message}\n\nThis is likely a CORS issue. To fix this:\n1. Make sure REACT_APP_ANTHROPIC_API_KEY is set in your CodeSandbox .env file\n2. Go to CodeSandbox → Server Control Panel → Secret Keys and add REACT_APP_ANTHROPIC_API_KEY`,
        },
      ]);
    }
  ...
  const handleImportVeterans = async () => {
    if (importPreview.length === 0) return;
    setIsImporting(true);
    let successCount = 0;
    let failCount = 0;

    for (const veteran of importPreview) {
      try {
        await addDoc(veteransRef, {
          ...veteran,
          userId: user.uid,
          userEmail: user.email,
          createdAt: serverTimestamp(),
        ...
        console.error("Failed to import veteran:", err);
        failCount++;
      }
    }
    ...
    await getVeterans();
    alert(
      `Import complete! ${successCount} veteran(s) added.${
  ...
  const exportReport = () => {
    ...
    const reportContent = ` 
Memorial Garden Registry Report 
...
Most Common Branch: ${reportData.overview.topBranch} Most Common Conflict: ${
...
${reportData.decades
  ...
  .join("\n")} 
--- 
Report generated by Memorial Garden Registry `;
...
<html> 
<head> 
<title>Memorial Garden Registry Report</title> 
<style> 
...
.stat-card { border: 1px solid #ddd; padding: 15px; border-radius: 5px; } .stat-number { font-size: 24px; font-weight: bold; color: #1e40af; } 
@media print { .no-print { display: none; } } 
</style> 
</head> 
<body> 
<h1>Memorial Garden Registry Report</h1> 
<p><strong>Generated:</strong> ${new Date().toLocaleDateString()}</p> 
<h2>Overview Statistics</h2> 
<div class="overview-grid"> 
<div class="stat-card"> 
<div>Total Veterans</div> 
<div class="stat-number">${reportData.overview.totalVeterans}</div> 
</div> 
<div class="stat-card"> 
<div>Recent Additions</div> 
<div class="stat-number">${reportData.overview.recentAdditions}</div> 
</div> 
<div class="stat-card"> 
...
<table> 
<tr><th>Branch</th><th>Count</th><th>Percentage</th></tr> ${reportData.branches
      .map(
        (item) =>
...
<table> 
<tr><th>Conflict/War</th><th>Count</th><th>Percentage</th></tr> ${reportData.conflicts
      .map(
        (item) =>
          `<tr><td>${item.name}</td><td>${item.count}</td><td>${(
            (item.count / reportData.overview.totalVeterans) *
...
<table> 
<tr><th>Rank</th><th>Count</th><th>Percentage</th></tr> ${reportData.ranks
      .map(
        (item) =>
          `<tr><td>${item.name}</td><td>${item.count}</td><td>${(
            (item.count / reportData.overview.totalVeterans) *
...
<table> 
<tr><th>Decade</th><th>Count</th><th>Percentage</th></tr> ${reportData.decades
      .map(
        (item) =>
          `<tr><td>${item.name}</td><td>${item.count}</td><td>${(
            (item.count / reportData.overview.totalVeterans) *
              ...
              <div>
                ...
                <div className="mt-8 pt-6 border-t border-gray-200 text-center text-gray-600">
                  ...
                  <p className="text-xs mt-1">
                    ...
                    records from the Memorial Garden Registry
                  </p>
                </div>
              </div>
            )}
        ...
        {activeTab === "assistant" && (
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <p className="text-gray-500 text-sm mb-4">
              Ask anything about veterans, military history, or the registry.
            </p>
            ...
            <div className="border border-gray-200 rounded-lg h-96 overflow-y-auto p-4 mb-4 bg-gray-50 flex flex-col gap-3">
              {chatMessages.length === 0 && (
                <div className="text-center text-gray-400 my-auto">
                  ...
                  <p>
                    Ask me about veterans, military branches, conflicts, or how
                    to use the registry.
                  </p>
                </div>
              )}
              ...
              {isChatLoading && (
                <div className="bg-white border border-gray-200 text-gray-500 self-start px-4 py-2 rounded-2xl rounded-bl-sm text-sm shadow-sm">
            ...
            <div className="flex gap-2">
              <input
                type="text"
                placeholder="Ask about veterans, military history, or the registry..."
                value={chatInput}
            ...
            {chatMessages.length === 0 && (
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    "How many veterans are in the registry?",
                    "What branches of the military are represented?",
                    "How do I add a veteran record?",
                    ...
                    <button
                      ...
                      className="text-xs bg-gray-100 text-gray-600 px-3 py-1 rounded-full hover:bg-blue-50 hover:text-blue-600 transition-colors"
                    >
                      {q}
                    </button>
                  ))}
        ...
        {activeTab === "about" && (
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <h2 className="text-3xl font-bold text-center mb-8 text-blue-800">
              {" "}
              About Memorial Garden Registry
            </h2>
            <div className="prose prose-lg max-w-none">
              <div className="bg-blue-50 border-l-4 border-blue-500 p-6 mb-8">
                ...
                <p className="text-gray-700 leading-relaxed">
                  The Memorial Garden Registry is a national digital archive
                  committed to preserving and honoring the memory of America’s
                  military veterans. This database serves as a centralized
              ...
              <div className="grid md:grid-cols-2 gap-8 mb-8">
                <div className="bg-gray-50 p-6 rounded-lg">
                  ...
                  <p className="space-y-2 text-gray-700">
                    Our purpose is to preserve military service records for
                    future generations, provide families with accessible
                    information, support genealogical and historical research,
                    and most importantly, honor the memory of those who served.
                    Through this lasting digital memorial, we ensure that no
                    hero is forgotten.
                  </p>
                </div>
                <div className="bg-gray-50 p-6 rounded-lg">
                  ...
                  <ul className="space-y-2 text-gray-700">
                    <li> Detailed veteran profiles</li>
                    <li> Branch, rank, and unit information</li>
                    <li> Conflict and service period data</li>
                    <li> Burial and cemetery locations</li>
                    <li> Secure, cloud-based preservation</li>
                  </ul>
                </div>
              </div>
              <div className="bg-green-50 border-l-4 border-green-500 p-6 mb-8">
                ...
                <p className="text-gray-700 leading-relaxed mb-4">
                  {" "}
                  Authorized users can contribute veteran information from
                  cemeteries nationwide. Each record includes essential details
                  such as:
                </p>
                <div className="grid md:grid-cols-3 gap-4 text-sm">
                  ...
                  <div className="bg-white p-3 rounded">
                    ...
                    <br />
                    Full name, birth and death dates, and service periods{" "}
                  </div>
                  <div className="bg-white p-3 rounded">
                    <strong>Military Service:</strong>
                    <br />
                    Branch, rank, unit, and conflicts served
                  </div>
                  <div className="bg-white p-3 rounded">
                    <strong>Memorial Details:</strong>
                    <br />
                    Burial location and memorial notes
                  </div>
                </div>
              </div>
              <div className="bg-red-50 border-l-4 border-red-500 p-6 mb-8">
                ...
                <p className="text-gray-700 leading-relaxed">
                  Our registry includes veterans from every era of American
                  military history— from the Revolutionary War to present-day
                  conflicts. Whether they served in the Army, Navy, Air Force,
                  Marines, Coast Guard, or Space Force, each veteran’s legacy
              ...
              <div className="text-center bg-gray-100 p-8 rounded-lg">
                ...
                <p className="text-gray-700 mb-6 max-w-3xl mx-auto">
                  We invite cemetery administrators, genealogists, historians,
                  and family members to contribute to our registry. By working
                  together, we can build a complete, respectful memorial that
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const field = e.target.value.substring(0, lastDash);
                      const order = e.target.value.substring(lastDash + 1);
                      setSortBy(field);
                      setSortOrder(order);
                    }}
                    className="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="mb-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <input
                    type="number"
                    step="any"
                    placeholder="e.g., 41.0950697"
                    value={veteranForm.latitude}
                    onChange={(e) =>
                      handleVeteranFormChange("latitude", e.target.value)
                    }
                    className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="number"
                    step="any"
                    placeholder="e.g., -73.5129976"
                    value={veteranForm.longitude}
                    onChange={(e) =>
                      handleVeteranFormChange("longitude", e.target.value)
                    }
                    className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <input
                  type="text"
                  placeholder="Cemetery Buried At"
                  value={veteranForm.cemeteryName}
                  onChange={(e) =>
                    handleVeteranFormChange("cemeteryName", e.target.value)
                  }
                  className="p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <input
            ...
            {/* ── Import Veteran Section ── always visible below the form */}
            <div className="mt-8 border-t-2 border-gray-200 pt-6">
              <div className="flex justify-between items-center mb-1">
                ...
                <button
                  onClick={() => {
                    const headers =
                      "firstName,middleName,lastName,branch,rank,conflictWar,dateOfBirth,dateOfDeath,serviceDateStart,serviceDateEnd,latitude,longitude,cemeteryName,additionalInfo";
                    const example =
                      "John,A,Smith,U.S. Army,Sergeant,Vietnam War,1940-05-01,2005-11-11,1962-06-01,1970-08-15,41.0950697,-73.5129976,Arlington National Cemetery,Awarded Bronze Star";
                    ...
                    a.href = url;
                    a.download = "veteran_import_template.csv";
                    document.body.appendChild(a);
                    a.click();
                    document.body.removeChild(a);
                    URL.revokeObjectURL(url);
                  }}
                  className="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-200 transition-colors text-sm border border-gray-300"
                >
                  ⬇ Download CSV Template
                </button>
              </div>
              <p className="text-sm text-gray-500 mb-5">
                Upload a <strong>.csv</strong> file or a <strong>.pdf</strong>{" "}
                exported from Google Sheets. CSV is parsed instantly; PDFs are
                read by AI to extract the table automatically.
              </p>

              {/* File Upload */}
              <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-6 mb-4 text-center hover:border-blue-400 transition-colors">
                <div className="text-3xl mb-2">📂</div>
                <p className="text-sm text-gray-600 mb-3">
                  Drag & drop or click to select a <strong>.csv</strong> or{" "}
              ...
              {/* Parsing Spinner */}
              {isParsing && (
                <div className="flex items-center gap-3 text-blue-600 mb-4 bg-blue-50 border border-blue-200 p-4 rounded-lg">
                  <svg
                  ...
                  <span className="text-sm font-medium">
                    {importFileType === "pdf"
                      ? "🤖 AI is reading the PDF and extracting veteran records — this may take a few seconds..."
                      : "Parsing CSV file..."}
                  </span>
                </div>
              )}
                  ...
                  <ul className="text-sm text-red-600 space-y-1">
                    {importErrors.map((err, i) => (
                      <li
                        key={i}
                        className={
                          err.startsWith("Possible fixes") || err.match(/^\d\./)
                            ? "ml-4 text-gray-600"
                            : ""
                        }
                      >
              ...
              {importPreview.length > 0 && (
                <div>
                      ...
                      <thead className="bg-gray-50 sticky top-0">
                        <tr>
                          {[
                            ...
                            <th
                              key={h}
                              className="px-3 py-2 text-left text-gray-600 font-semibold border-b whitespace-nowrap"
                            >
                              {h}
                            </th>
                          ))}
                  ...
                  <button
                    onClick={handleImportVeterans}
                    disabled={isImporting}
                    className="bg-green-500 text-white px-6 py-3 rounded-lg hover:bg-green-600 transition-colors disabled:bg-green-300 font-semibold"
                  >
                    {isImporting
                      ? `Importing... (${importPreview.length} records)`
                      : `✓ Confirm & Import ${importPreview.length} Veteran${
                ...
                {veterans.map((veteran) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      {veteran.latitude && veteran.longitude
                        ? `${veteran.latitude}, ${veteran.longitude}`
                        : "Not specified"}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        {veteran.cemeteryName || "Not specified"}
                      </div>
                      <div>
